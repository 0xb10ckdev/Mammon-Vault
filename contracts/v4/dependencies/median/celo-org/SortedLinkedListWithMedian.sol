...
library SortedLinkedListWithMedian {
  ...
  struct List {
    ...
    uint256 median;
    mapping(uint256 => MedianRelation) relation;
  }
  ...
  function insert(
    ...
    if (list.list.list.numElements == 1) {
      ...
      if (
        element.nextKey == 0 || list.relation[element.nextKey] == MedianRelation.Greater
      ) {
  ...
  function push(List storage list, uint256 key) internal {
    insert(list, key, 0, 0, list.list.list.tail);
  }
  ...
  function popN(List storage list, uint256 n) internal returns (uint256[] memory) {
    ...
    for (uint256 i = 0; i < n; i = i.add(1)) {
      uint256 key = list.list.list.head;
      keys[i] = key;
      remove(list, key);
    }
    return keys;
   ...
   */
  function getMedian(List storage list) internal view returns (uint256) {
    return list.median;
  }
  ...
  function getElements(List storage list)
    internal
    view
    returns (uint256[] memory, uint256[] memory, MedianRelation[] memory)
  {
